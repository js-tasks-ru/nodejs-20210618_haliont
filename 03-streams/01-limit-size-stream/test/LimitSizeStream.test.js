...
describe('streams/limit-size-stream', () => {
  describe('LimitSizeStream', () => {
    ...
    it('при проверке лимита должно учитываться количество байт, а не символов', (done) => {
      const smile = '😀';
      const limitStream =
        new LimitSizeStream({limit: Buffer.from(smile).length * 2 + 1, encoding: 'utf-8'});
      ...
      limitStream.on('close', () => {
        // eslint-disable-next-line max-len
        const message = `стрим должен выбросить ошибку используя событие 'error', возможно подсчет байт происходит неверно`;
        expect(limitStream.readableEnded, message).to.be.false;
      });
